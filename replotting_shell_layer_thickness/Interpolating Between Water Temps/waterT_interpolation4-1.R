...
for(i in 1:323){#imax = number of temperature points to start with
  if(row_count==1){waterfill[row_count]=water_temp4_1[water_count]
           water_count = water_count + 1
           row_count   = row_count + 1
  }
  else{
           if (sample(1:2,1)==1){
             if(num_5==178 & num_6==145){break}
             if (num_5<178){
               waterfill[row_count+gap1] <- water_temp4_1[water_count]
               water_count=water_count+1
             ...
             if (num_5>=178){
               waterfill[row_count+gap2] <- water_temp4_1[water_count]
               water_count=water_count+1
...
#out to a .csv file
etempVlt_me4_1 <- cbind(me4_1_lt$layer_thickness_mic,waterfill)
write.table(etempVlt_me4_1,file="Me4-1_waterfill.csv",append=FALSE,quote=FALSE,
